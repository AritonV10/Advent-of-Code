...
int main(void){
    ...
    printf("%d ", unpack32_signed(&g_cpu.REGISTER_B));
    printf("%d ", unpack32_signed(&g_cpu.REGISTER_C));
    printf("%d ", unpack32_signed(&g_cpu.REGISTER_D));
    ...
    for(i = 0; i < *n; ) {
        ...
        switch(get_opcode(g_cpu.IP)) {
            ...
            case JNZ:
                    ...
                    case 'd':
                        {
                            ...
                            if(i < *n)
                                printf("INFO: Jumping to instruction: [%d][%s]\n", i, (instr[i]));
                            else
                    ...
                    default:
                        {
                            ...
                            op_value = stoi(FIRST_OP, ' ');

                            i += (stoi(SECOND_OP, 0) * !!op_value + !op_value);
                ...
                break;
            case INVALID_OPCODE:
                fprintf(stderr, "ERR: Invalid instruction '%s'", g_cpu.IP);
                exit(EXIT_FAILURE);
                break;
...
get_opcode(const char *inst) {
    ...
    for(i = 0; i < NUM_OPCODES; ++i) {
        ...
        if(strcmp((const char *) &buf[0], g_opcodes[i].opcode) == 0)
