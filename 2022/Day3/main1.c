    ...
    (read(u32FD, &u8Char, 1 u)) != 0;)
  {
    if (u8Char == '\n')
    {
      ...
      for (; u8Idx < u8Len; ++u8Idx)
      {
        ...
        u8Bucket = ((au8Buf[u8Idx] >> 5 u) &0x01 u) +
          (u8Idx >= (u8Len >> 1)) *2;
         ...
         */
        u8Offset = ((0x5A u | (au8Buf[u8Idx] &0xF0 u)));
        printf(          "%d %c %d\n",
          u8Bucket,
          au8Buf[u8Idx],
          26 u - (u8Offset - au8Buf[u8Idx]));
        au32Vct[u8Group][u8Bucket] |=
          (1 u << (26 u - (u8Offset - au8Buf[u8Idx])));
      }

      #
      define AND(x, y)(x[u8Group][y] &x[u8Group][y + 2])

      for (u8Idx = 0; u8Idx < (sizeof(uint32_t) *8 u); ++u8Idx)
      {
        u32Sum +=
          (u8Idx + 26 u) *((AND(au32Vct, 0) &(1 u << u8Idx)) > 0);
        u32Sum += (u8Idx) *((AND(au32Vct, 1) &(1 u << u8Idx)) > 0);
      }

      u8Len = 0;
      u8Idx = 0;

      u8Group = (u8Group + 1) % 3 u;
      ...
      if (u8Group == 0)
      {
        ...
        for (; u8Idx < 32 u; ++u8Idx)
        {
          u32Sum2 +=
            (u8Idx + 26 u) *((au32Vct[0][0] &(1 u << u8Idx)) > 0);
          u32Sum2 += (u8Idx) *((au32Vct[0][1] &(1 u << u8Idx)) > 0);
        }
